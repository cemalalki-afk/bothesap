...
let aboneRoleId: string | null = null;  // 🛡️ Abone rolü burada tutuluyor
...
client.on("interactionCreate", async (interaction: Interaction) => {
  ...
  // 🛡️ ABONE ROLÜ KONTROL FONKSİYONU (Hazır)
  async function checkAbone(interaction: Interaction): Promise<boolean> {
    if (!aboneRoleId) {
      await interaction.reply({
        content: "❌ Sunucuda **Abone rolü** ayarlanmamış! Yöneticiden `/abonerol` ile ayarlamasını iste.",
        ephemeral: true,
      });
      return false;
    }
    ...
    const role = interaction.guild?.roles.cache.get(aboneRoleId);
    if (!role) {
      await interaction.reply({
        content: "❌ Ayarlanmış Abone rolü sunucuda bulunamadı! Lütfen tekrar `/abonerol` ile ayarla.",
        ephemeral: true,
      });
      return false;
    }
    ...
    if (!member.roles.cache.has(role.id)) {
      await interaction.reply({
        content: `🚫 Bu komutu sadece **${role.name}** rolüne sahip olanlar kullanabilir!`,
        ephemeral: true,
      });
      return false;
    }
  ...
  try {
    if (commandName === "hesap") {
      // 🛡️ ABONE KONTROLÜ (HESAP KOMUTU SADECE ABONELERE ÖZEL)
      if (!(await checkAbone(interaction))) return;
